...
// SendMachnetResponse sends a response to the NSaaS Channel.
...
func (s *Server) HandleAppendEntriesPipelineRecv(rpcId uint64, flow flow, start time.Time) error {
	...
	if pendingResponse, ok := s.pendingPipelineResponses[flow]; ok {
		...
		err := s.GetResponseFromChannel(pendingResponse.ch, flow, AppendEntriesPipelineRecv, rpcId, start)
		if err != nil {
			return err
		}
...
func (s *Server) HandleRPCs() {
	...
	for {
		...
		numReceived += 1
		glog.V(5).Info("Received message from Machnet: ", request.MsgType, " size: ", recvBytes, " numRecv: ", numReceived)
		now := time.Now()
		...
		if IsHeartbeat(request.Payload) {
			glog.Info("Received heartbeat from Machnet Channel.")
		...
		glog.Infof("HandleRpc: MsgType: %v RpcId:%v", request.MsgType, request.RpcId)
