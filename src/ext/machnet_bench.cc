...
const uint32_t kBufferSize = kPacketMTU + NSAAS_MSGBUF_SPACE_RESERVED;
...
BENCHMARK(BM_memcpy)->RangeMultiplier(2)->Range(8, NSAAS_MSG_MAX_LEN);
...
BENCHMARK(BM_memcpy_llc_thrash)
    ->RangeMultiplier(2)
    ->Range(8, NSAAS_MSG_MAX_LEN);

static void BM_nsaas_sendmsg(benchmark::State &state) {  // NOLINT
  ...
  std::vector<NSaaSRingSlot_t> slots(kRingSlotEntries - 1);

  // Lambda function to reset the buffer pool when the benchmark exhausts it.
  auto bufpool_reset = [&channel_ctx, &slots]() {
    // We need to drain the NSAAS application ring.
    uint32_t msg_pending = __nsaas_channel_app_ring_pending(channel_ctx);

    CHECK_EQ(__nsaas_channel_app_ring_dequeue(channel_ctx, msg_pending,
                                              slots.data()),
             msg_pending);

    // Next reset the buffer pool.
    // The NSaaS channel initialized the buffer pool with buffer
    // indexes from [0, capacity]. Since there is no API to perform this
    // operation we use this quick hack.
    uint32_t nbuffers_free = __nsaas_channel_buffers_avail(channel_ctx);
    CHECK_EQ(__nsaas_channel_buf_alloc_bulk(channel_ctx, nbuffers_free,
                                            slots.data(), nullptr),
             nbuffers_free);

    std::iota(slots.begin(), slots.end(), 0);
    CHECK_EQ(slots.size(), __nsaas_channel_buf_free_bulk(
                               channel_ctx, slots.size(), slots.data()));
  };

  for (auto _ : state) {
    for (uint32_t i = 0; i < state.range(1); i++) {
      int ret = nsaas_sendmsg(channel_ctx, &msghdr);
      if (ret != 0) {
...
static void CustomArguments(benchmark::internal::Benchmark *b) {
  for (uint32_t msg_sz = 8; msg_sz <= NSAAS_MSG_MAX_LEN; msg_sz *= 2) {
...
// BENCHMARK(BM_nsaas_sendmsg)->Range(4, 1500);
BENCHMARK(BM_nsaas_sendmsg)->Apply(CustomArguments);
